...
Este repositório reúne os códigos-fonte (scripts em R), as bases de dados agregadas em formato CSV e os gráficos gerados para o Trabalho de Conclusão de Curso (TCC) na Especialização em Docência da EPT do Instituto Federal do Rio Grande do Sul (IFRS) — Campus Sertão:
...
> 
> **Autor:** Luciano Marcos Paes  
> **Orientadora:** Profa. Dra. Ana Sara Castaman

O estudo analisa as contradições entre as exigências teórico-pedagógicas do Referencial Curricular Gaúcho do Ensino Médio (RCGEM), aprovado em 2021 sob a gestão de Eduardo Leite, e a materialidade concreta das escolas estaduais de Educação Profissional, caracterizadas por uma acentuada dependência de vínculos docentes temporários e precarizados.
...
1. **Universo de Análise:** Foram identificadas e analisadas **386 escolas estaduais** que ofertam Educação Profissional no estado.
2. **Taxa Média de Precarização:** Em média, **67,04%** do corpo docente de uma escola de Educação Profissional gaúcha é composto por professores temporários, atingindo uma mediana de **68,75%**.
3. **Dependência Crítica:** O esvaziamento do quadro funcional de carreira e a dependência de contratos precários atingem a escala de **85,24%** de todas as escolas gaúchas de EP (com mais de 50% de seus professores sob regime temporário), sendo que em **28,50%** delas a taxa de temporários supera a marca de **75%** de todo o quadro docente.
4. **Casos Extremos:** Instituições tradicionais estruturam-se com taxas críticas de pessoal temporário, tais como:
   * **Escola Estadual de Educação Básica Apeles Porto Alegre** (Porto Alegre): **96,67%** de vínculos temporários.
   * **Escola Estadual Técnica Agrícola Desidério Finamor** (Lagoa Vermelha): **95,24%** de vínculos temporários.
...
| Variável | Descrição |
| :--- | :--- |
| `co_entidade` | Código INEP identificador único da escola |
| `no_entidade` | Nome oficial da instituição escolar |
| `no_municipio` | Município de localização da escola |
| `qt_doc_bas_vinculo_concur` | Quantidade de docentes públicos concursados (efetivos) na escola |
| `qt_doc_bas_vinculo_contra` | Quantidade de docentes públicos contratados de forma temporária/emergencial |
| `perc_temporarios` | Taxa de professores sob regime temporário (temporários / total de docentes) |
| `doc_ept` | Quantidade total de docentes que atuam na Educação Profissional na unidade |
| `tur_ept` | Quantidade total de turmas ativas de Educação Profissional na unidade |
| `qt_doc_bas_disc_profissiona`| Quantidade de docentes alocados em disciplinas técnicas/profissionais |
...
Em estrita consonância com a **Lei Geral de Proteção de Dados (LGPD — Lei nº 13.709/2018)** e atendendo às políticas de segurança de dados reestruturadas pelo INEP desde 2024, **este repositório não armazena e nem processa dados microindividuais ou informações sensíveis que permitam a identificação direta ou indireta de estudantes ou professores**. 

Todas as bases disponíveis foram anonimizadas e agrupadas por **Unidade de Ensino (Escola)** [455], preservando integralmente o sigilo estatístico e o rigor ético-científico preconizados pela Portaria CNPq nº 2.664/2026.
